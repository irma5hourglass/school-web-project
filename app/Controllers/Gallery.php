...
class Gallery extends BaseController
{
    ...
    public function simpan()
    {
        if ($this->request->isAJAX()) {
            ...
            $valid = $this->validate([
                ...
                'sampul' => [
                    'label' => 'Sampul',
                    'rules' => 'uploaded[sampul]|max_size[sampul,2048]|is_image[sampul]',
                    'errors' => [
                        'uploaded' => 'sampul harus diunggah.',
                        'max_size' => 'Ukuran sampul maksimal 2MB.',
                        'is_image' => 'File yang diunggah harus berupa sampul.'
                    ]
                ]
            ]);
            ...
            if (!$valid) {
                ...
                $newName = $sampul->getName();

                \Config\Services::image()
                    ->withFile($sampul)
                    ->fit(800, 533, 'center')
                    ->save('img/sampul/thumb/' . 'thumb_' .  $sampul->getName());

                // Move the original image
                $sampul->move('img/sampul', $newName);


                $simpandata = [
                    'nama_gallery' => $this->request->getVar('nama_gallery'),
                    'slug_gallery' => url_title($this->request->getVar('nama_gallery'), '-', true),
                    'tgl_gallery' => date('Y-m-d'), // Use the current date
    ...
    public function update()
    {
        if ($this->request->isAJAX()) {
            ...
            if ($this->request->getFile('sampul')->isValid()) {
                $rules['sampul'] = [
                    'label' => 'Sampul',
                    'rules' => 'uploaded[sampul]|max_size[sampul,2048]|is_image[sampul]',
                    'errors' => [
                        ...
                        'is_image' => 'File {field} harus berupa sampul'
                    ]
                ];
            }
            ...
            if (!$valid) {
                ...
                // Cek apakah ada unggahan file sampul baru
                if ($this->request->getFile('sampul')->isValid()) {
                    $sampul = $this->request->getFile('sampul');
                    $newName = $sampul->getName();
                    \Config\Services::image()
    ...
    public function hapus()
    {
        if ($this->request->isAJAX()) {
            ...
            $gallery_id = $this->request->getVar('gallery_id');
            //check
            $this->gallery->find($gallery_id);
            $this->foto->hapusfoto($gallery_id);
    ...
    public function hapusall()
    {
        if ($this->request->isAJAX()) {
            ...
            for ($i = 0; $i < $jmldata; $i++) {
                //check
                $this->gallery->find($gallery_id[$i]);
                $this->foto->hapusfoto($gallery_id[$i]);
